...
// Modified by Damon Rocha, 2026/04/06
...
static esp_err_t motor_move_handler(httpd_req_t *req) {
  ...
  if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
    httpd_query_key_value(buf, "direction", direction, sizeof(direction));
  }
  if (strcmp(direction, "forward") == 0) move(true);
  else if (strcmp(direction, "backward") == 0) move(false);
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, direction, HTTPD_RESP_USE_STRLEN);
}

static esp_err_t motor_turn_handler(httpd_req_t *req) {
  char buf[50] = {0};
  char direction[10] = {0};
  if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
    httpd_query_key_value(buf, "direction", direction, sizeof(direction));
  }
  if (strcmp(direction, "left") == 0) turn(true);
  else if (strcmp(direction, "right") == 0) turn(false);
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, direction, HTTPD_RESP_USE_STRLEN);
}
...
static esp_err_t motor_wobble_handler(httpd_req_t *req) {
  wobble();
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "wobbling", HTTPD_RESP_USE_STRLEN);
}
...
void startCameraServer()
{
    ...
    httpd_uri_t motor_move_uri = {
    .uri = "/move",
    .method = HTTP_GET,
    .handler = motor_move_handler,
    .user_ctx = NULL
    };

    httpd_uri_t motor_turn_uri = {
    .uri = "/turn",
    .method = HTTP_GET,
    .handler = motor_turn_handler,
    .user_ctx = NULL
    };
    ...
    httpd_uri_t motor_wobble_uri = {
    .uri = "/wobble",
    .method = HTTP_GET,
    .handler = motor_wobble_handler,
    .user_ctx = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &motor_move_uri);
        httpd_register_uri_handler(camera_httpd, &motor_turn_uri);
        httpd_register_uri_handler(camera_httpd, &motor_stop_uri);
        httpd_register_uri_handler(camera_httpd, &motor_wobble_uri);
    }
